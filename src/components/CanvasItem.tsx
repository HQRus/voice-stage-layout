...
function ItemContent({ item, cornerRadius, boxShadow }: { item: PositionedItem; cornerRadius: number; boxShadow: string }) {
  ...
  switch (item.type) {
    ...
    case "video": {
      ...
      return (
        <div className="w-full h-full flex flex-col bg-card overflow-hidden" style={radiusStyle}>
          <div
          ...
          {hasCaption && (
            <div className="px-4 py-3 text-sm font-display font-medium flex items-center justify-between text-foreground/80 shrink-0">
        ...
        <div className={`w-full h-full ${shadow} bg-card flex flex-col overflow-hidden`} style={radiusStyle}>
          {image && (
            <div
              ...
              style={{
                height: Math.min(item.height * 0.45, 220),
                background: `${image.startsWith("http") ? `url(${image})` : image} center/cover no-repeat`,
              }}
            />
          )}
          <div className="p-6 flex flex-col gap-3 flex-1 min-h-0">
            ...
            <div className="text-2xl font-display font-bold text-foreground leading-tight line-clamp-2">{item.content}</div>
            {description && (
              <div className="text-base text-foreground/70 leading-relaxed line-clamp-3">{description}</div>
            )}
          </div>
        </div>
      );
